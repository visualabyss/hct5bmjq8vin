...
CATS = [
    "GAZE","EYES","MOUTH","SMILE","EMOTION",
    "YAW","PITCH","IDENTITY","QUALITY","TEMP","EXPOSURE"
]
...
def _pick_random(samples: List[Dict], n: int) -> List[Dict]:
    if not samples:
        return []
    k = min(n, len(samples))
    # unbiased selection; relies on global random seed
    ...
    for r in range(rows):
        for c in range(cols):
            ...
            try:
                ...
                # pose: accept either 'pose' or 'pose_deg' keys
                pose = rec.get('pose') or rec.get('pose_deg') or {}
                y = pose.get('yaw'); pi = pose.get('pitch')
                yv = float(y) if isinstance(y, (int, float)) else None
                pv = float(pi) if isinstance(pi, (int, float)) else None
                _overlay(src, label, p.name, yv, pv)
...
def main():
    ...
    if not man.exists():
        print("preview: manifest.jsonl missing.\nRun tag.py first.")
        return 2
