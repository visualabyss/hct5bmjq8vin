...
- Label column is fixed width (11 chars).
"""
import sys, time, shutil

PARTIALS = "▏▎▍▌▋▊▉"
BAR_WIDTH = 20  # exact length requested
...
class ProgressBar:
    ...
    def _line(self, done: int, fails: int | None = None, fps: float | None = None) -> str:
        ...
        fps_val = (done / max(1e-6, now - self._t0)) if fps is None else float(fps)
        # Fixed-width columns (11 for FAIL label+count; 8 for FPS field)
        ...
        # NOTE: 3 spaces between FPS and ETA
        return (
